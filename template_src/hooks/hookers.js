module.exports = function(ctx) {
    const Q = ctx.requireCordovaModule("q"),
        ...
        webpackDevServerPath = path.resolve(
            nodeModulesPath,
    ...
    const sys = {
        ...
        checkPackageName() {
            if (
                typeof packageJson.name === "undefined" ||
                packageJson.name === ""
            ) {
        ...
        checkManifestFile() {
            if (fs.existsSync(manifestFileSrcPath)) {
                ...
                fs.writeFileSync(
                    manifestFileCopyPath,
                    fs.readFileSync(manifestFileSrcPath)
                );
            }
        },
        ...
        checkNodeModules() {
            ...
            if (
                ...
                !fs.existsSync(path.resolve(nodeModulesPath, "cheerio/"))
            ) {
                ...
                exec(
                    ...
                    error => {
                        if (error) {
                            console.error(
                                `Error happened when npm install: ${error}`
                            );
                            defer.reject(
                                new Error(
                                    `Error happened when npm install: ${error}`
        ...
        makeNonDevServerChanges() {
            let defer = new Q.defer(),
                ...
                conf = cheerio.load(fs.readFileSync(configFile), {
                    xmlMode: true
                });
        ...
        makeDevServerChanges() {
            let defer = new Q.defer(),
                ...
                srcFile = path.resolve(
                    __dirname,
                    "../webpack/dev_helpers/device_router.html"
                ),
            ...
            $("body")
                .prepend(
                    `<script>const localServerIp = '${getRouterIpAddr()}'</script>`
                )
            ...
            if (conf("allow-navigation").length === 0)
                ...
                if (target.attr("href") !== "*")
                    target
                        .attr("data-href", target.attr("href"))
                        .attr("href", "*");
            }
        ...
        startWebpackBuild(isRelease) {
            ...
            let wpPath =
                webpackPath + (os.platform() === "win32" ? ".cmd" : "");

            exec(
                `"${wpPath}"` + (isRelease ? " --env.release" : ""),
                { cwd: pRoot, maxBuffer: 1024 * 1024 * 5 },
                error => {
                    if (error) {
                        console.error(
                            `Error happened when webpack build: ${error}`
                        );
                        defer.reject(
                            new Error(
                                `Error happened when webpack build: ${error}`
        ...
        addUpdate(p) {
            ...
            console.log(
                `${p} copied to ${path.resolve(targetStaticFolder, rel)}`
            );
        },
        ...
        addDir(p) {
            ...
            fs.mkdir(path.resolve(targetStaticFolder, rel), () => {
                console.log(
                    `Found ${p} folder. Created to ${path.resolve(
                        targetStaticFolder,
                        rel
                    )}`
                );
        ...
        delete(p) {
            ...
            fs.access(
                path.resolve(targetStaticFolder, rel),
                fs.constants.W_OK,
                err => {
                    if (!err) {
        ...
        sys
            ...
            .then(() => {
                if (
                    ...
                    return sys
                        .makeDevServerChanges()
                        .then(() => sys.startWebpackDevServer());
